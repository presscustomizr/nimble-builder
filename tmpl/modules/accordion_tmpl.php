...
if ( ! function_exists( 'Nimble\sek_print_accordion' ) ) {
  function sek_print_accordion( $accord_collec = array(), $accord_opts, $model ) {
      ...
      $is_multislide = count( $accord_collec ) > 1;
      $autoplay = ( ! skp_is_customizing() && true === sek_booleanize_checkbox_val( $accord_opts['autoplay'] ) ) ? "true" : "false";
      // don't authorize value < 300 ms
      $autoplay_delay = intval( $accord_opts['autoplay_delay'] ) < 300 ? 1000 : intval( $accord_opts['autoplay_delay'] );
      $pause_on_hover = true === sek_booleanize_checkbox_val( $accord_opts['pause_on_hover'] ) ? "true" : "false";;
      $loop_on = true === sek_booleanize_checkbox_val( $accord_opts['infinite_loop'] ) ? "true" : "false";
      $nav_type = ( is_string( $accord_opts['nav_type'] ) && !empty( $accord_opts['nav_type'] ) ) ? $accord_opts['nav_type'] : 'arrows_dots';
