// This color picker is a fixed + augmented + renamed version of /wp-admin/js/color-picker.js
...
// 3) renamed czr.czrColorPicker, instead of wp.wpColorPicker
...
( function( $, undef ) {
  ...
  ColorPicker = {
    ...
    open: function() {
      ...
      var self = this;
      $( 'body' ).trigger( 'click.wpcolorpicker' ).on( 'click.wpcolorpicker', function() {
            if ( self.element.iris('instance') ) {
                  self.close();
            }
      });
    },
  ...
  $.widget( 'czr.czrColorPicker', ColorPicker );
}( jQuery ) );
...
( function( $ ) {
  // Prevent double-init.
  if ( $.czr.czrColorPicker.prototype._hasAlpha ) {
    return;
  }
   ...
   */
  $.widget( 'czr.czrColorPicker', $.czr.czrColorPicker, {
    _hasAlpha: true,
    /**
     * @summary Creates the color picker.
     *
